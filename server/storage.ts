...
export class DatabaseStorage implements IStorage {
  ...
  async getCiVideoAnalysesEnriched(filters?: { hookType?: string, topicCategory?: string, minReplicationScore?: number, weekAdded?: string }) {
    const conditions = [];
    if (filters?.hookType) conditions.push(eq(ciVideoAnalyses.hookType, filters.hookType));
    if (filters?.topicCategory) conditions.push(eq(ciVideoAnalyses.topicCategory, filters.topicCategory));
    if (filters?.minReplicationScore) conditions.push(gte(ciVideoAnalyses.replicationScore, filters.minReplicationScore));
    ...
    const query = db
      .select({
        ...
        transcriptPreview: sql<string>`CASE WHEN length(${ciScrapedVideos.transcript}) > 200 THEN substring(${ciScrapedVideos.transcript} from 1 for 200) || '...' ELSE ${ciScrapedVideos.transcript} END`.as("transcript_preview"),
        // Competitor fields
      ...
      .leftJoin(ciCompetitors, eq(ciScrapedVideos.competitorId, ciCompetitors.id))
    ...
    return rows.map(r => ({
      ...
      transcript: null, // Don't send full transcripts in list view
    }));
  }
